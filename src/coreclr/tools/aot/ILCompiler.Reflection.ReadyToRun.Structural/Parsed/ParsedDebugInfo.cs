...
/// </summary>
public sealed class ParsedDebugInfo
{
    ...
    public static ParsedDebugInfo Parse(NativeReader imageReader, Machine machine, int majorVersion, int offset)
    {
        try
        {
            ...
            if (lookback != 0)
            {
                Debug.Assert(0 < lookback && lookback < offset);
            ...
            return new ParsedDebugInfo(bounds, variables, machine);
        }
        catch
        {
            return null;
    ...
    private static void ParseNativeVarInfo(
        NativeReader imageReader,
        int offset,
        Machine machine,
    ...
    private static int ReadEncodedStackOffset(NibbleReader reader, Machine machine)
    {
