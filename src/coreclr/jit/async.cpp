...
PhaseStatus Compiler::SaveAsyncContexts()
{
    ...
    if (!opts.IsOSR())
    {
        ...
        // Set the entry point so morph adds the R2R indirection cell arg on ARM64.
        // On x64 the cell is found from the return address so this is only needed for codegen.
